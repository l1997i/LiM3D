...
class Baseline(SemanticKITTI, prefix='baseline'):
    def __init__(self, split, config):
        ...
        self.load_file_paths = self.load_file_paths_full
        self.load_file_paths(split, self.label_directory)

    def load_file_paths_full(self, split='train', label_directory='labels'):
        self.lidar_paths = []
        self.label_paths = []
        for seq in self.config['split'][split]:
            ...
            lidar_paths = [os.path.join(dp, f) for dp, dn, fn in
                           ...
                           for f in fn if f.endswith('.bin') or chosen]
            ...
            if split != 'unsup':
                ...
                label_paths = [os.path.join(dp, f) for dp, dn, fn in 
                            ...
                            for f in fn if f.endswith('.label') or chosen]
    ...
    def get_label(self, idx):
        if self.split == 'unsup': return None
...
class ReflecCylindricalMT(Cylindrical, prefix='reflec_cylindrical_mt'):
    def __getitem__(self, idx):
        xyzr = self.get_lidar(idx)
        label = self.get_label(idx)
        return {
    ...
    def get_cylindrical_scene(self, xyzr, label, aug_methods):
        ...
        fea = np.concatenate((centered_rpz, rpz, xyz[:,:2], intensity.reshape(-1,1)), axis=1)
        fea = np.concatenate((fea, reflec_norm), axis=1)
        return torch.from_numpy(rpz_discrete), \
               torch.from_numpy(fea).float(), \
               torch.from_numpy(label).squeeze().long()
...
class ReflecPLSCylindricalMT(PLSCylindricalMT, prefix='reflec_pls_cylindrical_mt'):
    def get_cylindrical_scene(self, xyzr, label, aug_methods):
        xyz, intensity = xyzr[:,:3], xyzr[:,3]
        if self.split == 'train':
            xyz = self.augment(xyz, aug_methods)
        ...
        fea = np.concatenate((centered_rpz, rpz, xyz[:,:2], intensity.reshape(-1,1)), axis=1)
        fea = np.concatenate((fea, reflec_norm), axis=1)
        fea = np.concatenate((fea, super().pls(rpz_discrete[:,:2], label)), axis=1)
        return torch.from_numpy(rpz_discrete), \
               torch.from_numpy(fea).float(), \
               torch.from_numpy(label).squeeze().long()

class ReflecPLSCylindrical(PLSCylindricalMT, prefix='reflec_pls_cylindrical'):
    def __getitem__(self, idx):
        xyzr = self.get_lidar(idx)
        label = self.get_label(idx)
        return {
    ...
    def get_cylindrical_scene(self, xyzr, label, aug_methods, is_student=True):
        xyz, intensity = xyzr[:,:3], xyzr[:,3]
        if self.split == 'train':
            xyz = self.augment(xyz, aug_methods)
