...
class FeatureGenerator(nn.Module):
    ...
    def forward(self, feat, coord):
        ...
        feats = feat[0] #torch.cat(feat, dim=0)
        ...
        # Generate features
        p_feats = self.net(feats)           # point-based feats
        v_feats = torch_scatter.scatter_max(p_feats, unique_inv, dim=0)[0]
        v_feats = self.compress(v_feats)    # voxel-based feats
        p_feats = self.compress(p_feats)
        return p_feats, v_feats, unique_coords.type(torch.int64)
...
class AsymmetricUNet(nn.Module):
    def __init__(self,
        ...
        self.logits = SubMConv3d(4 * hid_feat, nclasses, indice_key="logit", kernel_size=3, stride=1, padding=1,
                                        bias=True)

    def forward(self, voxel_features, coors, batch_size):
        ret = SparseConvTensor(voxel_features, coors.int(), self.spatial_shape, batch_size)
...
class Cylinder3D(nn.Module):
    ...
    def forward(self, feat, coord, batch_size):
        p_feat, v_feat, coord = self.fcnn(feat, coord)
        return p_feat, self.unet(v_feat, coord, batch_size)  # need p_feat in UPL